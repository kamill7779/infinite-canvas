...
+ [新增] 渠道兼容Gemini格式。
+ [新增] 新增Codex App插件支持。
+ [新增] 图片生成改为异步队列处理，新增独立 worker 与 WebSocket 事件网关，支持生成进度推送与断线补偿查询。
+ [新增] Docker Compose 增加 Redis、worker、WebSocket 网关服务，生产环境强制使用真实 Redis。
+ [调整] 模型定价与渠道名建立数据库外键关系，渠道重名会被拦截，已停用渠道不会再参与模型选择与计价。
+ [优化] 生图工作台支持入队后实时回填结果，并在 WebSocket 不可用时使用状态查询兜底。
+ [修复] 生成队列新增 Prisma 迁移和 Node 构建脚本，避免生产库缺字段或构建依赖 Bun。
